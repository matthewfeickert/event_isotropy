...
ringSample = np.array(
    [ringGen(nList[i]) for i in range(5)]
)  # THESE ARE THE PHI VALUES OF THE PARTICLES
ringPtSample = np.array(
    [np.full(len(ringSample[i]), 1.0) for i in range(5)]
)  # THE UNORMALIZED WEIGHT: ALL OF EQUAL PT. NORMALIZATION IN EMD CALC
...
for i in range(5):
    ...
    for j in range(5):
        ...
        for num in range(1000):
            ringPoints2 = ringGenShift(
                nList[j]
            )  # The shift just randomly orients the ring, doesn't change particle spacing
            M = _cdist_phicos(
                ringPoints1, ringPoints2
            )  # M is distance in phi according to 1 - cos phi metric
