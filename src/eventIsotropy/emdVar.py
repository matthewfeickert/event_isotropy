...
def _cdist_phi(X, Y):
    ...
    phi2 = preproc(Y)
    phi_d = np.pi - np.abs(
        np.pi - np.abs(phi1[:, np.newaxis] - phi2[:])
    )  # GIVES MATRIX OF DIFFERENCE OF PHI VALUES
...
def _cdist_sphere(X, Y):
    theta_d = np.array(
        [
            [
                np.arccos(
                    np.around(np.dot(X[i], Y[j]), decimals=5)
                    / np.around(LA.norm(Y[j]) * LA.norm(X[i]), decimals=5)
                )
