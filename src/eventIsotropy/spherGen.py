...
def sphereAndDijet(nVal, djFrac):
    ...
    rotMat = np.array(
        [
            [
                ux * ux * (1 - np.cos(x)) + np.cos(x),
                ux * uy * (1 - np.cos(x)) - uz * np.sin(x),
                ux * uz * (1 - np.cos(x)) + uy * np.sin(x),
            ],
            [
                ux * uy * (1 - np.cos(x)) + uz * np.sin(x),
                uy * uy * (1 - np.cos(x)) + np.cos(x),
                uy * uz * (1 - np.cos(x)) - ux * np.sin(x),
            ],
            [
                ux * uz * (1 - np.cos(x)) - uy * np.sin(x),
                uy * uz * (1 - np.cos(x)) + ux * np.sin(x),
                uz * uz * (1 - np.cos(x)) + np.cos(x),
            ],
        ]
    )
    for j in range(numPix):
